...
/* clarified error message handler */
...
void init_logging(void) {
    unlink(LOG_DEST); /* don't care about success or error */
    /* even though "w" would overwrite previous entries, *
     * better to be positive the results are pure */
    if (freopen(LOG_DEST, "a", stderr) == NULL) { return; }

    /* _IOBLF = line buffered, BUFSIZ = sys alloc memory */
...
void flush_logs(err_lvl lvl) {
    ...
    if (!f) {
        fprintf(stdout, "failed to open %s to"
                    " read recorded logs\n", LOG_DEST);
        return;
    }
    ...
    char *line = NULL;
    size_t eol = 0;
    ssize_t n; /* signed so getline can return error */
